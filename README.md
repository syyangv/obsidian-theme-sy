...
### Type
- **Display / headings:** **Sims Sans Bold** (`fonts/Loucos_Lyne_-_thesimssansbold.otf`)
  — the real thing, user-provided. Falls back to Fredoka then Nunito.
- **Display small-caps:** **Sims Sans Bold SC** (`fonts/Loucos_Lyne_-_thesimssansboldsc.otf`)
  for eyebrows, tab labels, and HUD chrome where the in-game UI uses caps.
- **UI / body:** Nunito (Google Fonts) — the closest open-source match for the
  in-game body type; rounded terminals, open apertures. Sims Sans is display-
  only (single bold weight) so body copy needs a multi-weight partner.
- **Mono:** JetBrains Mono — for code blocks and frontmatter.
